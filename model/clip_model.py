...
class VisionTransformer(nn.Module):
    ...
    #@torch.no_grad
    def conv_embedding(self, input_image: torch.Tensor):
        x = self.conv1(input_image)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        
        return x
    
    def forward_body_pe(self, x: torch.Tensor, positional_embedding: torch.Tensor):
    ...
    def forward_body(self, x: torch.Tensor):
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    if vit:
        ...
        #pe = copy.deepcopy(state_dict["visual.positional_embedding"])
    else:
    ...
    # pe = copy.deepcopy(model.visual.positional_embedding)
    # resize modified pos embedding
    model.load_param(state_dict)
    return model, model_cfg#, pe
