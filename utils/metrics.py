...
class Evaluator():
    ...
    def _compute_embedding(self, model:IRRA):
        model = model.eval()
        ...
        if "token" in model.current_task:
            token_feats = torch.cat(token_feats, 0)
            return qfeats, gfeats, qids, gids, token_feats
        else:
            return qfeats, gfeats, qids, gids
    
    def eval(self, model, i2t_metric=False):
        if "token" in model.current_task:
            qfeats, gfeats, qids, gids, token_feats = self._compute_embedding(model)
        else:
        ...
        top1 = t2i_cmc[0]
        if "token" in model.current_task:
            mean_token = (gfeats+token_feats)/2
            mean_token = F.normalize(mean_token, p=2, dim=1) # image features
            similarity_token = qfeats @ mean_token.t()
            t2i_cmc, t2i_mAP, t2i_mINP, _ = rank(similarity=similarity_token, q_pids=qids, g_pids=gids, max_rank=10, get_mAP=True)
            t2i_cmc, t2i_mAP, t2i_mINP = t2i_cmc.numpy(), t2i_mAP.numpy(), t2i_mINP.numpy()
            table.add_row(['t2i_token_mean', t2i_cmc[0], t2i_cmc[4], t2i_cmc[9], t2i_mAP, t2i_mINP])
            
            mean_token = token_feats
            mean_token = F.normalize(mean_token, p=2, dim=1) # image features
            similarity_token = qfeats @ mean_token.t()
            t2i_cmc, t2i_mAP, t2i_mINP, _ = rank(similarity=similarity_token, q_pids=qids, g_pids=gids, max_rank=10, get_mAP=True)
            t2i_cmc, t2i_mAP, t2i_mINP = t2i_cmc.numpy(), t2i_mAP.numpy(), t2i_mINP.numpy()
            table.add_row(['t2i_token', t2i_cmc[0], t2i_cmc[4], t2i_cmc[9], t2i_mAP, t2i_mINP])
